      ...
      <div className="border-b border-black/[0.06] bg-[#f5f5f7]">
        <div className="mx-auto flex max-w-[1200px] flex-col gap-8 px-5 py-12 md:flex-row md:items-center md:px-10 lg:px-12">
          ...
          <div>
            ...
            <p className="mt-2 text-[15px] text-[#6e6e73]">{[company.industry, company.city, company.scale].filter(Boolean).join(" · ")}</p>
            {company.website ? (
      ...
      <div className="mx-auto max-w-[1200px] space-y-16 px-5 py-14 md:px-10 lg:px-12">
          ...
          {company.articles.length === 0 ? (
            ...
            <ul className="mt-4 space-y-3">
              {company.articles.map((a) => (
                <li key={a.slug}><Link href={`/articles/${a.slug}`} className="text-[15px] font-medium text-[var(--accent)] transition hover:opacity-75">{a.title}</Link></li>
              ))}
            </ul>
          )}
        </section>
        <section>
          ...
          {company.jobs.length === 0 ? (
            ...
            <ul className="mt-4 space-y-3">
              {company.jobs.map((j) => (
                <li key={j.id}><Link href={`/jobs/${j.id}`} className="text-[15px] font-medium text-[var(--accent)] transition hover:opacity-75">{j.title}</Link></li>
              ))}
            </ul>
          )}
        </section>
