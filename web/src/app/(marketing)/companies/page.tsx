...
export default async function CompaniesPage() {
  const companies = await safeQuery(() => getPublicCompaniesList(), [], "CompaniesPage");
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5 py-14 md:px-10 md:py-20 lg:px-12">
      ...
      <h1 className="mt-2 text-[2.25rem] font-semibold tracking-tight text-[#1d1d1f] md:text-[2.5rem]">企业名录</h1>
      <p className="mt-3 max-w-xl text-[15px] leading-relaxed text-[#6e6e73]">已通过审核的入驻企业。</p>
      {companies.length === 0 ? (
          ...
          {companies.map((c) => (
            <Link key={c.slug} href={`/companies/${c.slug}`}
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-[12px] text-[#86868b]">{[c.industry, c.city].filter(Boolean).join(" · ")}</p>
                {c.description ? <p className="mt-2 line-clamp-2 text-[13px] leading-relaxed text-[#6e6e73]">{c.description}</p> : null}
              </div>
            </Link>
          ))}
        </div>
