...
async function CompaniesList() {
  ...
  if (companies.length === 0) {
    return <p className="mt-12 text-[15px] text-[#6e6e73]">暂无入驻企业。</p>;
  }
...
function ListSkeleton() {
  return (
    <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 animate-pulse">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="flex gap-4 rounded-2xl border border-black/[0.06] bg-white p-5">
          <div className="h-14 w-14 shrink-0 rounded-xl bg-[#f5f5f7]" />
