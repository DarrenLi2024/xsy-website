...
async function JobsList() {
  ...
  if (jobs.length === 0) {
    return <p className="mt-12 text-[15px] text-[#6e6e73]">暂无招聘职位。</p>;
  }
        ...
        <li key={j.id}>
          <div className="flex flex-col gap-1 px-6 py-5 transition-colors hover:bg-[#f5f5f7]/80 md:flex-row md:items-center md:justify-between">
            <div>
              <Link
                href={`/jobs/${j.id}`}
                className="text-[16px] font-semibold text-[#1d1d1f] transition hover:text-[var(--accent)]"
              >
                {j.title}
              </Link>
              <p className="mt-1 text-[13px] text-[#6e6e73]">
                <Link href={`/companies/${j.company.slug}`} className="font-medium text-[var(--accent)] transition hover:opacity-75">
                  {j.company.name}
                </Link>
                {j.city ? ` · ${j.city}` : ""}
              </p>
  ...
  return (
    <div className="mt-12 animate-pulse space-y-3">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="flex items-center justify-between rounded-2xl border border-black/[0.06] bg-white px-6 py-5">
          <div className="space-y-2">
            <div className="h-4 w-48 rounded bg-[#f5f5f7]" />
            <div className="h-3 w-32 rounded bg-[#f5f5f7]" />
          </div>
          <div className="h-3 w-16 rounded bg-[#f5f5f7]" />
        </div>
      ))}
    </div>
  );
