...
export default async function AwardsPage() {
  ...
  return (
    <div className="mx-auto max-w-[720px] px-5 py-14 md:px-8 md:py-20 lg:px-10">
      ...
      <p className="mt-3 text-[15px] leading-relaxed text-[#6e6e73]">年度评选与奖项说明。</p>
      {campaigns.length === 0 ? (
        <p className="mt-12 text-[15px] text-[#6e6e73]">暂无评选活动。</p>
      ) : (
          ...
          {campaigns.map((c) => (
            <li key={c.id}>
              <div className="rounded-2xl border border-black/[0.06] bg-white p-6 shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
                ...
                <p className="mt-2 text-[13px] text-[#86868b]">{c.year} · {c.startDate.toLocaleDateString("zh-CN")} — {c.endDate.toLocaleDateString("zh-CN")}</p>
                {c.summary ? <p className="mt-4 text-[15px] leading-relaxed text-[#424245]">{c.summary}</p> : null}
                <Link href={`/awards/${c.slug}`} className="mt-6 inline-block text-[14px] font-semibold text-[var(--accent)] transition hover:opacity-75">查看详情</Link>
              </div>
            </li>
          ))}
        </ul>
