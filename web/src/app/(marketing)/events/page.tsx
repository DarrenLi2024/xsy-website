...
const typeLabels: Record<string, string> = {
  EXHIBITION: "展会", CONFERENCE: "峰会", WEBINAR: "线上",
  SALON: "沙龙", WORKSHOP: "培训",
};
...
async function EventsList() {
  ...
  if (events.length === 0) {
    return <p className="mt-12 text-[15px] text-[#6e6e73]">暂无进行中的活动。</p>;
  }
        ...
        <li key={e.id}>
          <Link href={`/events/${e.id}`}
            ...
            <div className="flex flex-col p-6">
              <span className="w-fit rounded-full border border-black/[0.06] bg-[#f5f5f7] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#6e6e73]">
      ...
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="overflow-hidden rounded-2xl border border-black/[0.06] bg-white">
          ...
          <div className="space-y-3 p-6">
            ...
            <div className="h-5 w-3/4 rounded bg-[#f5f5f7]" />
